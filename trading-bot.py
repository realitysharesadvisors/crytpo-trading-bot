...
def main(argv):
    ...
    while True:
        ...
        ##Error for wrong period of time##
            ...
            for point, ma in zip(dataPoints, maPoints):
        ...
        maPoints.append({'date':dataDate, 'price': str(currentMovingAverage)})

        ##timestamp##
        print(
            # "%s Period: %ss %s: %s Moving Average: %s" % (dataDate, period, pair, lastPairPrice, currentMovingAverage))
            "Previousprice: %s , %s: %s Moving Average: %s" % (previousPrice, pair, lastPairPrice, currentMovingAverage))
